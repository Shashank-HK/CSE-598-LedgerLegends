...
contract product {
    ...
    mapping(bytes32 => uint256) public productTimestamps;
    ...
    function addProduct(
        ...
        productTimestamps[_productSN] = block.timestamp;
    }
        ...
        )
    {
        ...
        uint256[] memory times = new uint256[](productSNs.length);
        for (uint i = 0; i < productSNs.length; i++) {
            ...
            times[i] = productTimestamps[productSNs[i]];
        }
        return (productSNs, sellerCodes, manufacturerCodes, times);
    }
